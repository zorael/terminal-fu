...
rotate_colour() {
	[[ $USE_COLOURS ]] || return
	[[ ${_COLOUR_PREV:-36} -gt 35 ]] && _COLOUR_PREV=31 || _COLOUR_PREV=$((_COLOUR_PREV+1))
...
get_colour_glyph() {
	...
	case ${1:-0} in
		...
		CYAN)		colour=36 ;;
		CLEAR)		clear=0  ;;
		[0-9][0-9])	colour=$1 ;; # can use pattern matching but would need extglob
		*)
...
summary() {
	local IFS listed_files skipped_dirs skipped_files string_numfiles string_skip_files string_skip_dirs
	listed_files=${#_FILES[@]}
	...
	printf -- "\n%d %s listed, with %d %s and %d %s skipped\n" $listed_files $string_numfiles \
		$skipped_files $string_skip_files $skipped_dirs $string_skip_dirs
}
